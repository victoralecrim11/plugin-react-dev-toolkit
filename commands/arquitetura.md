...
Leia `devLevel` em `dashboard-config.json` (`GET /api/config`) e use-o como teto de complexidade. Não pergunte o nível: ele foi definido no `/setup`. Se o perfil não existir, aplique o fallback da skill `dashboard-projetos`.
